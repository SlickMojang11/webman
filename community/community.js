...
async function saveHistory(commitMsg) {
  ...
  try {
    // Re-fetch SHA to avoid 422 conflicts on rapid sequential writes
    const r0 = await fetch(API_BASE, {
      headers: { 'Authorization': auth, 'Accept': 'application/vnd.github.v3+json' }
    });
    if (r0.ok) { const d0 = await r0.json(); historySha = d0.sha; }
...
async function saveHistoryWithSubmissionToken(commitMsg) {
  ...
  try {
    // Always re-fetch the current SHA before committing to avoid 422 conflicts
    // (a previous post/reply already incremented the SHA on GitHub)
    const r0 = await fetch(API_BASE, {
      headers: { 'Authorization': auth, 'Accept': 'application/vnd.github.v3+json' }
    });
    if (r0.ok) { const d0 = await r0.json(); historySha = d0.sha; }
