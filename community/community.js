...
/* ===== CORE SAVE (with retry on SHA conflict) ===== */

async function fetchCurrentSha(auth) {
  // Force a fresh response — no CDN cache, no stale sha
  const headers = { 'Accept': 'application/vnd.github.v3+json', 'Cache-Control': 'no-cache' };
  if (auth) headers['Authorization'] = auth;
  try {
    const r = await fetch(API_BASE + '?t=' + Date.now(), { headers });
    if (r.ok) { const d = await r.json(); return d.sha || null; }
  } catch(_) {}
  return null;
}

async function commitHistory(auth, commitMsg) {
  if (!auth) return { ok: false, msg: 'No credentials. Set a Submission Token in the Admin Panel.' };

  // Always get the freshest SHA right before writing
  const fresh = await fetchCurrentSha(auth);
  if (fresh) historySha = fresh;
  ...
  try {
    ...
    if (r.ok) {
      const d = await r.json();
      // Update our sha from the actual response so next write has it immediately
      historySha = d.content?.sha ?? historySha;
      return { ok: true };
    }

    // 422 = SHA conflict — the file changed under us. Retry once with a fresh sha.
    if (r.status === 422) {
      const freshRetry = await fetchCurrentSha(auth);
      if (freshRetry && freshRetry !== historySha) {
        historySha = freshRetry;
        body.sha = historySha;
        ...
        if (r2.ok) {
          ...
          historySha = d2.content?.sha ?? historySha;
          return { ok: true };
        }
        const e2 = await r2.json().catch(() => ({}));
        return { ok: false, msg: `Save failed after retry (${r2.status}): ${e2.message || ''}` };
      }
    }

    const errBody = await r.json().catch(() => ({}));
    return { ok: false, msg: `GitHub ${r.status}: ${errBody.message || 'unknown error'}` };
  } catch(e) {
...
async function saveHistory(commitMsg) {
  const auth = getAuthHeader();
  const result = await commitHistory(auth, commitMsg);
...
async function saveHistoryWithSubmissionToken(commitMsg) {
  const subTok = (() => {
    try { return window.parent?.localStorage?.getItem('wm_submission_token') || localStorage.getItem('wm_submission_token'); }
    catch(_) { return null; }
  })();
